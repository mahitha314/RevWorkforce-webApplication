...
public class TeamLeavesController {
	...
	public ResponseEntity<ApiResponse> approve(@PathVariable Long managerId, @PathVariable Long leaveId,
			@RequestParam(required = false) String comments) {

		ApiResponse response = service.approveLeave(managerId, leaveId, comments);

		return ResponseEntity.status(response.getStatus()).body(response);
	}

	@PostMapping("/reject/{managerId}/{leaveId}")
	public ResponseEntity<ApiResponse> reject(@PathVariable Long managerId, @PathVariable Long leaveId,
			@RequestParam String comments) {

		ApiResponse response = service.rejectLeave(managerId, leaveId, comments);
	...
	public ResponseEntity<ApiResponse> calendar(@PathVariable Long managerId) {
		...
		ApiResponse response = service.getTeamLeaveCalendar(managerId);
	...
	public ResponseEntity<ApiResponse> balance(@PathVariable Long managerId) {
		...
		ApiResponse response = service.getTeamLeaveBalance(managerId);
