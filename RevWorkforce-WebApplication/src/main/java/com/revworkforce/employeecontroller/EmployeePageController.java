...
public class EmployeePageController {
    ...
    public String dashboard(Model model) {
        ...
        List<LeaveBalance> balances =
                leaveBalanceRepo.findByEmployee_Id(id);

        List<LeaveRequest> leaveRequests =
                leaveRequestRepo.findByEmployee_Id(id);

        List<Goal> goals =
                goalRepository.findByEmployee_Id(id);

        List<PerformanceReview> reviews =
                performanceReviewRepo.findByEmployee_Id(id);

        int totalAllocated = balances.stream()
                .mapToInt(LeaveBalance::getTotalLeaves)
                .sum();

        int totalUsed = balances.stream()
        ...
        int pendingCount = (int) leaveRequests.stream()
                .filter(r -> r.getLeaveApproval() == null)
                .count();

        int goalPercentage = goals.isEmpty()
                ? 0
                : goals.stream().mapToInt(Goal::getProgress).sum() / goals.size();
