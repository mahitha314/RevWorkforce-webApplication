...
public class EmployeePageController {
	...
	public EmployeePageController(EmployeeRepository employeeRepo, LeaveBalanceRepository leaveBalanceRepo,
			LeaveTypeRepository leaveTypeRepo, LeaveRequestRepository leaveRequestRepo, GoalRepository goalRepository,
			AnnouncementRepository announcementRepo, NotificationRepository notificationRepo,
			PerformanceReviewRepository performanceReviewRepo, HolidayRepository holidayRepository,
	...
	public String updateProfile(@ModelAttribute Employee employee, @RequestParam(required = false) String oldPassword,
	...
	public String submitLeave(@RequestParam Long leaveTypeId, @RequestParam String startDate,
			@RequestParam String endDate, @RequestParam String reason, RedirectAttributes redirectAttributes) {
	...
	public String saveReview(@ModelAttribute PerformanceReview review, RedirectAttributes redirectAttributes) {
	...
	public String submitReview(@PathVariable Long id, RedirectAttributes redirectAttributes) {
	...
	public String notificationsPage(Model model) {
		...
		model.addAttribute("notifications", notificationRepo.findByEmployee_IdOrderByCreatedAtDesc(employee.getId()));
	...
	public String dashboard(Model model) {
		...
		long pendingRequests = requests.stream().filter(r -> r.getLeaveApproval() == null
				|| (r.getLeaveApproval() != null && "Pending".equalsIgnoreCase(r.getLeaveApproval().getStatus())))
				.count();

		List<PerformanceReview> reviews = performanceReviewRepo.findByEmployee_Id(employee.getId());

		double performanceRating = reviews.stream().filter(r -> r.getManagerRating() > 0)
				.mapToDouble(PerformanceReview::getManagerRating).average().orElse(0);
