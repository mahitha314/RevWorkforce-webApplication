...
public class LeaveManagementServiceImpl implements LeaveManagementService {
    ...
    public ApiResponse getAllLeaveTypes() {
        return new ApiResponse(200, "Leave types fetched",
    ...
    public ApiResponse assignLeave(LeaveBalanceDTO dto) {
     ...
     // 🔔 SEND NOTIFICATION
        ...
        notification.setMessage("Admin assigned " + dto.getTotalLeaves() + " leaves to you");
    ...
    public ApiResponse adjustLeave(AdjustLeaveDTO dto) {
     ...
     // 🔔 NOTIFICATION
        NotificationDTO notification = new NotificationDTO();
        notification.setEmployeeId(balance.getEmployee().getId());
        notification.setTitle("Leave Balance Updated");
        notification.setMessage("Admin adjusted your leave balance. Reason: " + dto.getReason());
        notification.setType("LEAVE");
        notification.setStatus("ACTIVE");
        notification.setReferenceId(balance.getId());
        ...
        return new ApiResponse(200,
                "Leave adjusted successfully. Reason: " + dto.getReason(),
                balance);
    }

    @Override
    public ApiResponse getAllEmployeeLeaves() {
        return new ApiResponse(200, "All employee leave details",
                leaveBalanceRepository.findAll());
    }

    @Override
    public ApiResponse getEmployeeLeave(Long empId) {
        return new ApiResponse(200, "Employee leave details",
                leaveBalanceRepository.findByEmployeeId(empId));
    }

    @Override
    public ApiResponse getDepartmentReport(Long deptId) {
        return new ApiResponse(200, "Department leave report",
                leaveBalanceRepository.findByDepartmentId(deptId));
    }
