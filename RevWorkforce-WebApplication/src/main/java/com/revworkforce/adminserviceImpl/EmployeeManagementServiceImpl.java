...
public class EmployeeManagementServiceImpl implements EmployeeManagementService {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(EmployeeManagementServiceImpl.class);
    ...
    public EmployeeManagementServiceImpl(
        ...
        logger.info("EmployeeManagementServiceImpl initialized");
    }

    @Override
    public ResponseEntity<ApiResponse> addEmployee(EmployeeDTO dto) {

        logger.info("Attempting to add employee with email: {}", dto.getEmail());
        ...
        Department department = departmentRepository.findById(dto.getDepartmentId())
                .orElseThrow(() -> {
                    logger.error("Department not found: {}", dto.getDepartmentId());
        ...
        Designation designation = designationRepository.findById(dto.getDesignationId())
                .orElseThrow(() -> {
                    logger.error("Designation not found: {}", dto.getDesignationId());
        ...
        if (dto.getManagerId() != null) {
            manager = employeeRepository.findById(dto.getManagerId())
                    .orElseThrow(() -> {
                        logger.error("Manager not found: {}", dto.getManagerId());
        ...
        activityLogService.log(
                "Added Employee",
                "Employee Management",
                "Added employee " + employee.getFirstName(),
                "SUCCESS",
                request
        );
    ...
    public ResponseEntity<ApiResponse> updateEmployee(String employeeId, EmployeeDTO dto) {
        ...
        logger.info("Updating employee with employeeId: {}", employeeId);

        Employee existing = employeeRepository.findByEmployeeId(employeeId)
                .orElseThrow(() -> {
                    logger.error("Employee not found for update: {}", employeeId);
                    return new RuntimeException("Employee not found");
                });
        ...
        logger.info("Employee updated successfully: {}", existing.getEmployeeId());
    ...
    public ResponseEntity<ApiResponse> deleteEmployee(String employeeId) {
        ...
        logger.warn("Deleting employee with employeeId: {}", employeeId);
    ...
    public ResponseEntity<ApiResponse> deactivateEmployee(String employeeId, String reason) {
        ...
        logger.warn("Deactivating employee: {}", employeeId);
